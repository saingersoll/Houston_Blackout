...
**st_intersects vs st_difference vs st_within**

The current output is a "sparse geometry binary predicate list"
...
# identifying homes boardering the buffer and within the buffer
houston_blackout_highways <- st_intersection( highway_buffer, cropped_houston_blackout)
...
```{r include=TRUE}
# Filtering for homes withing the blackout zone

# blackout_buildings <- st_intersects(buildings, hb_boundary_plus)
houston_blackout_buildings <- st_intersection(buildings, houston_blackout_highways)
...
income <- st_read("data/ACS_2019_5YR_TRACT_48_TEXAS.gdb", layer = "X19_INCOME") %>% 
  dplyr::select(c('GEIOD', 'B19013e1'))%>% 
  mutate(GEOID_Data = GEOID,
         median_income = B19013e1)

income <- st_transform(3803)
```
...
# Cropping the data 
cropped_income <- cropped_income[houston, .pred = st_intersects]

# Census tracts that experienced blackout
blackout_census <- cropped_income[blackout_buildings,] %>% 
  mutate(blackout = 'yes')

# Combining Cropped datums
blackout_combined <- full_join(as.data.frame(cropped_income), as.data.frame(blackout_census))

blackout_combined$blackout[is.na(blackout_combined$blackout)] <- "no"

no_blackout <- st_as_sf(blackout_combined[!grepl("yes",blackout_combined$blackout),])
```
