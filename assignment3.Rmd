...
```{r}
houston_blackout_highways3 <- houston_blackout_mask[highway_buffer,] 

plot(houston_blackout_highways3)
```
...
houston_blackout_buildings <- st_intersection(houston_blackout_highways, buildings) 
# the number of homes impacted
houston_blackout_homes <- nrow(houston_blackout_buildings)
houston_blackout_homes
```
...
-   find which census tracts had blackouts\
    **psuedocode need to test once upper chunks run**

```{r include=TRUE}

# Combining the data to region of interest
census_income <- left_join(socioeconomic, income) %>%
  st_transform('EPSG:3083') 
...
blackout_census <- houston_income[houston_blackout_buildings,] %>% 
  mutate(blackout = 'yes')
...
tm_shape(houston_income) + 
  ...
  tm_dots() +
  tm_layout(title = "Median Census Tract Income in Houston Effected by Blackouts")
