...
A cropped version of the blackout map was created using `st_difference` , this mask of the region of interest was then re-projected to EPSG:3083 (NAD83 / Texas Centric Albers Equal Area).

```{r}
# cropping blackout locations to the houston dimensions
# this slices the vector_blackout_locations to match houston data, everything on the line is sliced
# This provides data where Houston is masked
houston_blackout_mask <- st_difference(vector_blackout_locations, houston)
...
houston_blackout_highways <- st_difference(houston_blackout_mask, highway_buffer) %>% 
  st_make_valid(houston_blackout_highways)
#%>% 
 # st_make_valid(houston_blackout_highways)
...
Using the `st_intersects` function to filter for homes beyond the buffer that were effected by blackouts.

-   filter to homes within blackout areas\

-   count number of impacted homes -\> should be closure to 7k

```{r include=TRUE}
# Filtering for homes beyond the buffer zone
#houston_blackout_homes <- houston_blackout_highways[ buildings,] 
# the number of homes impacted
#nrow(houston_blackout_homes)


# Filtering for homes beyond the buffer zone
houston_blackout_homes1 <- st_intersection(houston_blackout_highways, buildings) 
# the number of homes impacted
nrow(houston_blackout_homes1)
...
# Cropping the data 
houston_income <- census_income[houston,] %>% 
  mutate(geometry = houston_income$Shape)

# Census tracts that experienced blackout
blackout_census <- houston_blackout_homes1[houston_income,] %>% 
  mutate(blackout = 'yes')

# Combining Cropped datums
houston_blackout_combined <- full_join(as.data.frame(houston_income), as.data.frame(blackout_census))
...
tm_shape(houston_income) + 
  ...
  tm_title(text = "Median Census Tract Income in Houston Effected by Blackouts", size = 1) 
...
blackout_map <-  tm_shape(houston_income) + 
  tm_polygons(fill = "median_income", 
          title = "Median Income ($)",
          palette = "magma") +
  tm_borders() +
  tm_title(text = "Median income for census tracts that had blackouts") +
...
no_blackout_map <-  tm_shape(no_blackout) + 
  tm_polygons(fill = "median_income", 
          ...
          palette = "magma") +
  tm_borders() +
  tm_title(text = "Median income for census tracts that did not have blackouts") +
   tm_scalebar(position = c('right','bottom')) + 
  tm_compass( type = 'rose', 
              lw = 0.,
              size = 5) 
...
```{r}
ggplot(data = houston_blackout_combined, aes(x = blackout, y = median_income, col = blackout)) +
  geom_jitter(width = 0.1) +
  geom_boxplot(alpha = 0.5) +
  labs(x = "Blackout", y = "Median Income", title = "Houston Power Outage Compared to Median Income Distribution")
```
