...
houston_blackout <- st_difference(vector_blackout_locations, houston)
...
cropped_houston_blackout <- cropped_houston_blackout %>% 
  st_transform(3083) %>% 
  st_make_valid(cropped_houston_blackout)
```
...
houston_blackout_highways <- st_difference(cropped_houston_blackout, highway_buffer) %>% 
...
houston_blackout_buildings <- buildings[houston_blackout_highways,]
...
```{r}
plot(houston_blackout_buildings)
```
...
socio_income  <- left_join(socioeconomic, income) %>%
...
# Cropping the data 
houston_income <- census_income[houston, .predicate = st_intersects]
